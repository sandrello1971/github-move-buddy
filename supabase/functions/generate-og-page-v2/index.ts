...
serve(async (req) => {
  ...
  try {
    ...
    const ogImage =
      post.featured_image ||
      `${siteUrl}/lovable-uploads/9afc0cc7-085e-45e9-8a5d-eaccf88663b6.png`;
    const description =
      post.excerpt || `${post.title} - Leggi l'articolo completo su Sabadvance`;
...
<body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; text-align: center; padding: 50px; background: #f5f5f5;">
  ...
  <script>window.location.replace("${postUrl}");</script>
    ...
    // Ensure old metadata is not reused
    ...
    console.log("Generated OG page (v2):", publicUrlData.publicUrl);
